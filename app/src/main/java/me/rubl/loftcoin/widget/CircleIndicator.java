...
public class CircleIndicator extends RecyclerView.ItemDecoration {
    ...
    public void onDrawOver(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        if (adapter != null) {
            float totalWidth = 2f * indicatorRadius * adapter.getItemCount();
            float posX = (parent.getWidth() - totalWidth) / 2f;
            float posY = parent.getHeight() - 2f * indicatorRadius;
    ...
    private void drawIndicator(@NonNull Canvas canvas, float x, float y, boolean active) {
        canvas.drawCircle(x, y, indicatorRadius, active ? activePaint : inactivePaint);
    }
}
